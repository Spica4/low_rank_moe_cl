...
# module_id -> List[Tensor([...1])] （呼ばれるたびに追記）
_gw_captures: dict[int, list] = defaultdict(list)
_original_compute = None


def _patch_gating():
    """LanguageGuidedGating.compute_gating_weights をパッチして GW を収集する。"""
    global _original_compute
    _original_compute = LanguageGuidedGating.compute_gating_weights

    def _debug_compute(self, x, text_embedding):
        result = _original_compute(self, x, text_embedding)
        _gw_captures[id(self)].append(result.detach().cpu().float())
        return result

    LanguageGuidedGating.compute_gating_weights = _debug_compute


def _restore_gating():
    """パッチを元に戻す。"""
    global _original_compute
    if _original_compute is not None:
        LanguageGuidedGating.compute_gating_weights = _original_compute
        _original_compute = None
...
def collect_stats(model: SwinUNETRMoE, patches: torch.Tensor):
    ...
    # --- 収集した GW を layer_key × step_idx ごとに集約 ---
    ...
    # --- ルーティング決定を計算 ---
    stats: dict[str, dict] = {}
    for layer_key, step_dict in layer_gws.items():
        if 0 not in step_dict or 1 not in step_dict:
            continue

        gw0 = np.concatenate(step_dict[0])  # 全 GW_step0 値 (1D)
        gw1 = np.concatenate(step_dict[1])  # 全 GW_step1 値 (1D)

        # 長さが合わない場合は短い方に揃える（window sizeの違いによる）
